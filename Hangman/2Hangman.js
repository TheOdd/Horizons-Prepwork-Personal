...
hangman.isGuessWrong = function(word, guess) {
  if (guess.length != 1)
    throw "Guess can only be a single letter!";
...
hangman.getRandomWord = function() {
  return hangman.words[hangman.getRandomInteger(0, 6)];
}
...
hangman.getGameStatus = function(word, guesses) {
  var numWrong = 0;
  var uniqWord = word.split("").filter(function(i, u, s) {
  ...
  var numRight = guesses.length - numWrong;
  if (numWrong >= hangman.maxWrongGuesses)
    return "lose";
  if (numRight == uniqWord.length)
    return "win";
  else
    return "in_progress";
}
...
hangman.makeGuess = function(letter) {
  if (hangman.getGameStatus(hangman.game.word, hangman.game.guesses) != "in_progress")
    throw "There is no game in progress!";
