...
towers.isValidBoard = function(board) {
  ...
  if (!towers.isArray(board) || board.length != 3) // Checks if board is not array or if it doesn't contain exactly three elements
    return false;
  for (var i = 0; i < board.length; i++) { // Loop through first dimension of array
    if (!towers.isArray(board[i]) || board[i].length > 3) // Check if each element in first dimension is array and if it contains over 3 elements
      return false;
    for (var x = 0; x < board[i].length; x++) { // Loop through second dimension of array
      if (board[i][x] > 2 || board[i][x] < 0) // Check if not 0, 1, or 2
        return false;
      if (x > 0 && board[i][x] > board[i][x-1]) // Check if greater than element before in array
        return false;
      if (board[i][x] == 0)  //////////////////////////////////////////
        zero++;              //                                      //
      if (board[i][x] == 1)  // Increment once each time accordingly //
        one++;               // if 0, 1, or 2 is present in array.   //
      if (board[i][x] == 2)  //                                      //
        two++;               //////////////////////////////////////////
    }
  }
  if (zero != 1 || one != 1 || two != 1) // Checks for presence of 0, 1, and 2 *only once* in entire array.
    return false;
  else
    return true;
}
...
towers.makeMove = function(fromTower, toTower) {
  var testBoard = towers.copyBoard(towers.game.board); // Board to test move
  var towerOne = testBoard[fromTower]; // First tower (fromTower)
  var towerTwo = testBoard[toTower]; // Second tower (toTower)
  var popped = towerOne.pop(); // Getting value of and removing last element of towerOne.
  towerTwo.push(popped); // Pretty much just "moving" last element from towerOne to top of towerTwo.
  if (towers.isValidBoard(testBoard)) // Check if valid board
    towers.game.board = testBoard; // If valid, set this board as the new global game board
  else
    throw "Something went wrong!"; // Otherwise, throw error.
  towers.drawGame(testBoard); // Draw new board
  if (testBoard[0] == "" && testBoard[1] == "") // Test if the new board is equal to the win-condition array
    return towers.win(); // If true, then return win function
}
