...
toolbox.lastIndexOf = function(array, item) {
  var inversedIndex = toolbox.find(array.reverse(), function(n) {
    return n == item;
  });
  if (inversedIndex == -1)
    return -1;
  else
    return array.length - inversedIndex - 1;
}
...
toolbox.difference = function(array1, array2) {
  return toolbox.filter(array1, function (item1) {
    for (var i = 0; i < array2.length; i += 1) {
      if (item1 == array2[i])
        return false;
      else if (item1 != array2[i] && i == array2.length - 1)
        return true;
    }
  });
}
